...
a=this.collection.where({cover_photo:$(a.target).attr("id")});1===a.length&&SouthRidge.Router.navigate("photos/"+a[0].get("id")+"/"+a[0].get("name"),{trigger:!0})}});
SouthRidge.Views.PhotosView=Backbone.View.extend({el:"#content",collection:null,albumName:"",initialize:function(a){_.bindAll(this,"render");this.collection=a.collection;this.albumName=a.albumName;this.render()},render:function(){SouthRidge.Utils.ScrollTop();var a={albumName:decodeURIComponent(this.albumName),photos:this.collection.models},a=_.template($("#photos").html(),a);SouthRidge.Utils.SetTopBar("");forge.topbar.addButton({text:"Back",position:"left",tint:[59,118,38,255]},function(){SouthRidge.Router.navigate("albums",
{trigger:!0})});SouthRidge.Utils.DoneLoading();$(this.el).unbind().html(a).show()},events:{"tap div.photo, div.photoTablet":"handleTap"},handleTap:function(a){a.preventDefault();a=this.collection.where({id:$(a.target).attr("id")});1===a.length&&(a=a[0].get("source"),forge.tabs.openWithOptions({url:a,tint:[59,118,38,255]},function(a){SouthRidge.Utils.Log(a.url)}))}});
